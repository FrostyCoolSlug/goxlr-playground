...
impl TUSBAudio<'_> {
    ...
    pub fn event_loop(
        ...
        if callbacks.ready_notifier.send(()).is_err() {
        ...
        loop {
            ...
            if wait_result != WAIT_TIMEOUT {
                ...
                loop {
                    ...
                    if event_response[0] == 1 && event_response[1] == 1 && event_response[2] == 0 {
                        if callbacks.device_event.capacity() > 0 {
                            let se = callbacks.device_event.blocking_send(());
                            if se.is_err() {
            ...
            if let Err(_error) = self.get_device_id_string(handle) {
                ...
                if new_handle.is_err() {
                    ...
                    let _ = callbacks.device_event.blocking_send(());
...
pub struct EventChannelSender {
    pub(crate) ready_notifier: oneshot::Sender<()>,
    pub(crate) data_read: Sender<bool>,
    pub(crate) device_event: Sender<()>,
}
